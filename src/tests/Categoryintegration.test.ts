...
describe("Category CRUD Integration", () => {
  ...
  // Unknown types map to Expense group (see CategoryController.createCategory)
  it("maps unknown category type to Expense group and creates successfully", async () => {
    const res = await controller.createCategory({
      body: { name: "Bkash", type: "Payment Method" },
    } as any);

    expect(res.statusCode).toBe(201);
    expect((res.body as { type: string }).type).toBe("Expense");
  });
