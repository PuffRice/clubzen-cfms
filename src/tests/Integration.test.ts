...
describe("Integration — Income & Expense flows", () => {
  ...
  it("should carry payment_method through controller → service → domain and survive retrieval (income)", async () => {
    const income = await txController.addIncome(
      2500,
      new Date("2026-02-15"),
      "Sponsorship",
    ...
    expect(income.payment_method).toBe("one-time");

    const incomes = await txService.getIncomes();
    expect(incomes[0].payment_method).toBe("one-time");
    const rows = await repo.findAll();
    expect(rows[0].payment_method).toBe("one-time");
  });

  it("should carry payment_method through controller → service → domain and survive retrieval (expense)", async () => {
    const expense = await txController.addExpense(
      750,
      new Date("2026-02-20"),
      "Catering",
    ...
    expect(expense.payment_method).toBe("bank-transfer");

    const rows = await repo.findAll();
    expect(rows[0].payment_method).toBe("bank-transfer");

    const expenses = await txService.getExpenses();
    expect(expenses[0].payment_method).toBe("bank-transfer");
  });
